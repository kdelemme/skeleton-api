          ...
          return directives.password.comparePassword(req.body.password, rows[0].password)
            .then(function(match) {
              ...
              if (tokenRow && tokenRow[0] && tokenRow[0].count === '0') {
                return [token, knexClient('tokens').insert({user_id: rows[0].id, token: token/*, expiry: Date.now() + directives.token.defaultExpiry*/ })];
              }
              else {
                return [token, knexClient('tokens').where({user_id: rows[0].id}).update({ token: token/*, expiry: Date.now() + directives.token.defaultExpiry*/ })];
              }
